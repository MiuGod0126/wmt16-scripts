...
if __name__ == '__main__':
    ...
    new_pair_lang=lang_id_filter(pairs,model_path='lid.176.bin',src_lang=args.src_lang,tgt_lang=args.tgt_lang,threshold=0.4)
    write_text_pair(new_pair_lang,out_src_file=out_src_file,out_tgt_file=out_tgt_file)


    # # src_lang,tgt_lang=sys.argv[1],sys.argv[2]
    # # src_file,tgt_file=sys.argv[3],sys.argv[4]
    # pairs=read_text_pair(src_file,tgt_file)
    # # new_pair=length_filter(pairs,low=1,up=250,ratio=1.5)
    # new_pair_lang=lang_id_filter(new_pair,model_path='lid.176.bin',src_lang=src_lang,tgt_lang=tgt_lang,threshold=0.4)
    # write_text_pair(new_pair_lang,out_src_file=f"{src_lang}.txt",out_tgt_file=f"{tgt_lang}.txt")

    # pairs=read_text_pair('data/train_tiny.zh','data/train_tiny.en')
    # new_pair=length_filter(pairs,low=1,up=250,ratio=1.5)
    # new_pair_lang=lang_id_filter(new_pair,model_path='lid.176.bin',src_lang='zh',tgt_lang='en',threshold=0.4,report_neg=True)
    # write_text_pair(new_pair_lang,out_src_file='zh.txt',out_tgt_file='en.txt')
